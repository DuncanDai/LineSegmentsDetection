...
static const mxArray *b_emlrt_marshallOut(const real_T u_data[], const int32_T
  u_size[1]);
static void c_emlrt_marshallIn(const emlrtStack *sp, const mxArray *final_labels,
...
void returnLines_api(const mxArray * const prhs[2], int32_T nlhs, const mxArray *
                     plhs[5])
{
  ...
  real_T (*begin_points_data)[40];
  real_T (*end_points_data)[40];
  real_T (*endpositions_data)[20];
  real_T (*lslength_data)[20];
  real_T (*startpositions_data)[20];
  int32_T begin_points_size[2];
  int32_T end_points_size[2];
  int32_T endpositions_size[1];
  int32_T lslength_size[1];
  int32_T startpositions_size[1];
  st.tls = emlrtRootTLSGlobal;
  begin_points_data = (real_T (*)[40])mxMalloc(sizeof(real_T [40]));
  end_points_data = (real_T (*)[40])mxMalloc(sizeof(real_T [40]));
  lslength_data = (real_T (*)[20])mxMalloc(sizeof(real_T [20]));
  startpositions_data = (real_T (*)[20])mxMalloc(sizeof(real_T [20]));
  endpositions_data = (real_T (*)[20])mxMalloc(sizeof(real_T [20]));
  ...
  returnLines(&st, points_proj, final_labels, *begin_points_data,
              ...
              *lslength_data, lslength_size, *startpositions_data,
              startpositions_size, *endpositions_data, endpositions_size);

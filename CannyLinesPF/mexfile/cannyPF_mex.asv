...
//         unsigned char *imgPtr = image.data;
        const TypedArray<double> imageArray = std::move(inputs[0]);   // Issue: inputs[0] needs to the same uint8 type, otherwise, Can't convert the Array(double) to this TypedArray
...
//         stream << std::endl ; displayOnMATLAB(stream);
        ...
        outputs[1] = factory.createCellArray<Array> ({edgeChains.size(), size_t(1)});

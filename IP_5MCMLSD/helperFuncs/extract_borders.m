...
elseif strcmp(decision_criterion, 'len/num')  
    idx = 5;
    [left_maxVal, ~] = max(left_valid_windows_features(:, idx));  % if all values are NaN -> left_maxVal is NaN -> left_maxIndex is 0
    [right_maxVal, ~] = max(right_valid_windows_features(:, idx));
else
    error('the decision_criterion should be "num", "len" or extend features');
end

if (idx == 5) & (isnan(left_maxVal))
    left_border_pos = 0;
else

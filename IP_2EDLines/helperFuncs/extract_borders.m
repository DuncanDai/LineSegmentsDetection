function  [left_pos1, left_pos2, right_pos1, right_pos2, windows_features, full_edges_filter_by_angle] = extract_borders(edgeLines, resizeImageHeight, resizeImageWidth, windowWidth, windowStepSize, ...
...
left_valid_windows_features = windows_features( windows_features(:, 2) <= valid_left_boundary, : );  % windowEnd
right_valid_windows_features = windows_features( windows_features(:, 1) >= valid_right_boundary, : );  % windowStart
...
if (idx == 6) & (isnan(left_maxVal))
    left_pos1 = 0;  left_pos2 = 0; 
else
    ...
    if left_valid_windows_features(left_window_index, 3) == 0  % the number of edge lines is 0
        ...
        max_window_start = left_valid_windows_features(left_window_index, 1);  
        max_window_end = left_valid_windows_features(left_window_index, 2);  
        max_inds = filter_by_window(full_edges_filter_by_angle, max_window_start, max_window_end);
...
if (idx == 6) & (isnan(right_maxVal))
    ...
    if right_valid_windows_features(right_window_index, 3) == 0  % the number of edge lines is 0
        right_pos1 = 0; right_pos2 = 0;
    else
%         right_border_pos = right_valid_windows_features(right_window_index, 1) + windowWidth/2;  
        max_window_start = right_valid_windows_features(right_window_index, 1);  
        max_window_end = right_valid_windows_features(right_window_index, 2);  
        max_inds = filter_by_window(full_edges_filter_by_angle, max_window_start, max_window_end);
        max_window_with_edgeLines = full_edges_filter_by_angle(max_inds, 1:4); % 4 elements: (x1, y1, x2, y2)

...
public:
    void operator()(ArgumentList outputs, ArgumentList inputs) {
        ...
        for(i=0; i<rows; ++i)
            for(j=0; j<cols; ++j)
                image.at<uchar>(i, j) = (int) (imageFromMatlab[i][j] * 255);  // from matlab(column major order (row, col)) to C++(row major order (col, row)) 
        ...
        cout << "Running time in C++ = " << time * 1000 << "ms" << endl;  //output: ms
        cout << "Running time in OpenCV = " << timeEnd * 1000  << "ms" << endl;  //output: ms
        /////////////////////////////////////////
        ...
        TypedArray<int> edgeLinesInMatlab = factory.createArray<int> ({edgeLinesNumber, 4}); 

        for(size_t i=0; i<edgeLinesNumber; ++i){
